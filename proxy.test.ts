...
/**
 * Proxy security-header tests (Next.js 16 renamed `middleware` to `proxy`).
 *
 * The proxy adds a CSP + a few hardening headers to every page
...
describe('proxy — security headers', () => {
  ...
  it('adds Strict-Transport-Security on HTTPS origins', () => {
    const res = proxy(makeReq('https://atp-tour-santiago.onrender.com/'));
  ...
  it('does NOT add Strict-Transport-Security on HTTP origins (dev)', () => {
    const res = proxy(makeReq('http://localhost:3000/'));
